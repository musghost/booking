...
k.month=i[parseInt(l[1])-1];k.day=parseInt(l[0]);return k;},h=[],g=function(k){var j=$("<div class='flight-info-sidebar'>"),i='<div class="head"><h2><i></i>Información de vuelo</h2></div>';
j.append(i);$.each(k,function(n,m){var l=$("<div class='body'>"),q=$("<h3>"),o=parseInt(m.stopover),p=$("<h3 class='scale'>");if(n===0){q.attr("id","title-search");
}q.html("Voy");l.append(q);$.each(m.destinations,function(s,r){var u=$("<ul class='airp'>"),t=$("<h3 class='date'>");t.html(r.date);u.append("<li><span>"+r.from+"</span><span>"+r.hourDep+"</span></li>");
u.append("<li class='middle'><i></i></li>");u.append("<li><span>"+r.to+"</span><span>"+r.hourArr+"</span></li>");l.append(t);l.append(u);});if(o===0||o<0){p.html("Sin escalas");
}else{if(o==1){p.html("1 escala");}else{p.html(o+" escalas");}}l.append(p);j.append(l);});j.append(f());$("#cart-component").prepend(j);},f=function(){var k=$("<div class='total-price'>"),i=$("<ul>"),j="";
$(".cartPrices").last().find(".price-type").each(function(){var l=$("<li>"),m=$(this).find("span").eq(0).html(),n=$(this).find(".price-value");if(n.length>0){n=$(this).find(".price-value").first().html();
}else{n="";}l.append("<span>"+m+"</span>","<span>"+n+"</span>");i.append(l);});$("#cart-total-price .prices-alternative > span").each(function(l){if(l==1){j+="<span class='amount'>"+$(this).html()+"</span>";
}if(l==0){j+="<span class='currency'>"+$(this).html()+"</span>";}});i.append("<li><span>"+$("#total-price-label").html()+"</span><span>"+j+"</span></li>");
k.append(i);return k;};$(".cart-departing, .cart-arriving").each(function(){var j=[],l=$(this).find("li.city"),m=$(this).find("dd"),i=0,k=l.length-2;l.each(function(o){var s=$(this).clone(),r=l.eq(o-1).clone();
...
g(h);$("#modify-search-trigger").addClass("modify");};var a=function(){var f=$("#booking-process li").last();if($("#dtcontainer-outbounds ul.airp").length<1){$("#dtcontainer-outbounds table tr").each(function(i){$(this).find(".prices-amount").each(function(){$(this).prepend("<span>$</span>");
