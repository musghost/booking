...
k.month=i[parseInt(l[1])-1];k.day=parseInt(l[0]);return k;},h=[],g=function(m){var l=$("<div class='flight-info-sidebar'>"),j='<div class="head"><h2><i></i>Información de vuelo</h2></div>',i=$(".component-section.cart-pax").find("span"),k="",n=$("<h3 class='passenger'>");
...
if($("#dtcontainer-outbounds ul.airp").length<1){$("#dtcontainer-outbounds table tr").each(function(o){$(this).find(".prices-amount").each(function(){$(this).prepend("<span>$</span>");
});if(o===0){var g=$(this).find("th");g.eq(1).hide();g.eq(2).hide();g.eq(3).hide();g.eq(4).hide();g.eq(5).hide();g.eq(0).width("190px");}else{var i=$(this).find("td");
i.eq(5).hide();i.eq(4).hide();i.eq(1).hide();i.eq(2).hide();i.eq(3).hide();var n=i.eq(2).find("span").html(),m=i.eq(3).find("span").html(),h=$.trim(i.eq(4).html());
var k=$("<ul class='airp'>"),l=$('<h3 class="scale">');k.html("<li><span>"+c[0]["nodea"]+"</span><span>"+n+'</span></li><li class="middle"><i></i></li><li><span>'+c[0]["nodeb"]+"</span><span>"+m+"</span></li>");
l.html("Sin escalas");if($(this).attr("id")==="flight-advisory-outbound-row"){$(this).remove();return;}i.eq(0).prepend(k,l);var j=$(this).find("a").html();
$(this).find("a").addClass("info-plus").html("<i></i><span>M&aacute;s informaci&oacute;n de vuelo ("+j+")</span>");}});$("#cart-component .component-top").append($("#fare-rules-anchor"),$("#modify-search-trigger"));
$(".flight-info-sidebar").remove();b();$(".flight-info-sidebar").first().append($("#fare-rules-anchor"));$("#title-search").append($("#modify-search-trigger"));
}if($("#dtcontainer-inbounds ul.airp").length<1){$("#dtcontainer-inbounds table tr").each(function(i){$(this).find(".prices-amount").each(function(){$(this).prepend("<span>$</span>");
