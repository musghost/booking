(function(a,b){function cy(a){return f.isWindow(a)?a:a.nodeType===9?a.defaultView||a.parentWindow:!1;}function cv(a){if(!ck[a]){var b=c.body,d=f("<"+a+">").appendTo(b),e=d.css("display");
...
});})(window);$(document).ready(function(){(function(){var e=false;$("#booking-process").addClass("active");$("#booking-process li").each(function(f){$(this).addClass(((f+1)%2===0)?"two":"one");
...
f.nodea=g.substr(1,3);f.nodeb=e.substr(1,3);c.push(f);});var b=function(){var f=function(k){var j=["January","February","March","April","May","June","July","August","September","October","November","December"],m=k.split("/"),l={};
l.month=j[parseInt(m[1])-1];l.day=parseInt(m[0]);return l;},i=[],e=function(){var o=window.innerHeight,k=o-$("#footer").height(),j=$("#cart-component").height(),n=$("body").scrollTop(),l=$("#ROOT").height();
...
}}},h=function(n){var m=$("<div class='flight-info-sidebar'>"),k='<div class="head"><h2><i></i>Flight Information</h2></div>',j=$(".component-section.cart-pax").find("span"),l="",o=$("<h3 class='passenger'>");
j.each(function(p){if(p===0||p===1){l+=$(this).html();}});o.html(l);m.append(k);$.each(n,function(r,q){var p=$("<div class='body'>"),v=$("<h3>"),t=parseInt(q.stopover),u=$("<h3 class='scale'>"),s=$("<span class='flight-num'>");
if(r===0){v.attr("id","title-search");}v.html(q.actionTo);p.append(v);$.each(q.destinations,function(x,w){var z=$("<ul class='airp'>"),y=$("<h3 class='date'>");
y.html(w.date);z.append("<li><span>"+w.from+"</span><span>"+w.hourDep+"</span></li>");z.append("<li class='middle'><i></i></li>");z.append("<li><span>"+w.to+"</span><span>"+w.hourArr+"</span></li>");
p.append(y);p.append(z);});if(t===0||t<0){u.html("Nonstop");}else{if(t==1){u.html("1 layover");}else{u.html(t+" layovers");}}s.html(q.flightNumber);p.append(s);
p.append(u);m.append(p);});m.append(o);m.append(g());$("#cart-component").prepend(m);},g=function(){var l=$("<div class='total-price'>"),j=$("<ul>"),k="";
$(".cartPrices").last().find(".price-type").each(function(){var m=$("<li>"),o=$(this).find("span").eq(0).html(),p=$(this).find(".price-value"),n=$(this).find(".toggler a").first().html(),r=$("<a href='#' class='view-details'>"),q=$(this).find(".price-element-details").first().clone(),t=$("<span>"),s=$("<span>");
...
l.append(j);return l;};$(".cart-departing, .cart-arriving").each(function(){var k=[],n=$(this).find("li.city"),o=$(this).find("dd"),j=0,m=$(this).find("h4").first().html(),q=$(this).find("li.number").first(),l=n.length-2;
var p=q.andSelf().contents().filter(function(){return this.nodeType===3;})[1];if(typeof p==="undefined"){p="";}else{p=$.trim(q.find("span").first().html()+p.textContent);
}n.each(function(s){var w=$(this).clone(),v=n.eq(s-1).clone();w.children().remove();v.children().remove();w=$.trim(w.html()).substr(1,3);v=$.trim(v.html()).substr(1,3);
if(s>0){var u=f(o.eq(j).html().split(" ")[2]),t=o.eq(j++).html().substr(0,7),r=o.eq(j++).html().substr(0,7);destination={from:v,to:w,date:u.day+", "+u.month,hourDep:t,hourArr:r};
k.push(destination);}});departing={stopover:l,destinations:k,actionTo:m,flightNumber:p};i.push(departing);});h(i);$("#modify-search-trigger").addClass("modify");
};var a=function(){var f=$("#booking-process li").last();if($("#dtcontainer-outbounds ul.airp").length<1){$("#dtcontainer-outbounds table tr").each(function(i){$(this).find(".prices-amount").each(function(){$(this).prepend("<span>$</span>");
...
var l=$("<ul class='airp'>"),j=$('<h3 class="scale">');l.html("<li><span>"+c[0]["nodea"]+"</span><span>"+h+'</span></li><li class="middle"><i></i></li><li><span>'+c[0]["nodeb"]+"</span><span>"+g+"</span></li>");
j.html("Nonstop");if($(this).attr("id")==="flight-advisory-outbound-row"){$(this).remove();return;}m.eq(0).prepend(l,j);$(this).find("a").addClass("info-plus").html("<i></i><span>Additional information</span>");
...
m.eq(5).hide();m.eq(4).hide();m.eq(1).hide();m.eq(2).hide();m.eq(3).hide();var h=m.eq(2).find("span").html(),g=m.eq(3).find("span").html(),n=$.trim(m.eq(4).html());
var l=$("<ul class='airp'>"),j=$('<h3 class="scale">');l.html("<li><span>"+c[0]["nodeb"]+"</span><span>"+h+'</span></li><li class="middle"><i></i></li><li><span>'+c[0]["nodea"]+"</span><span>"+g+"</span></li>");
j.html("Sin escalas");if($(this).attr("id")==="flight-advisory-inbound-row"){$(this).remove();return;}m.eq(0).prepend(l,j);$(this).find("a").addClass("info-plus").html("<i></i><span>Additional information</span>");
}});$("#cart-component .component-top").append($("#fare-rules-anchor"),$("#modify-search-trigger"));$(".flight-info-sidebar").remove();b();$(".flight-info-sidebar").first().append($("#fare-rules-anchor"));
...
m.eq(5).hide();m.eq(4).hide();m.eq(1).hide();m.eq(2).hide();m.eq(3).hide();var h=m.eq(2).find("span").html(),g=m.eq(3).find("span").html(),n=$.trim(m.eq(4).html());
var l=$("<ul class='airp'>"),j=$('<h3 class="scale">');l.html("<li><span>"+c[0]["nodea"]+"</span><span>"+h+'</span></li><li class="middle"><i></i></li><li><span>'+c[0]["nodeb"]+"</span><span>"+g+"</span></li>");
j.html("Sin escalas");if($(this).attr("id")==="flight-advisory-both-row"){$(this).remove();return;}m.eq(0).prepend(l,j);$(this).find("a").addClass("info-plus").html("<i></i><span>Additional information</span>");
...
$(this).html(e+" seats left!");});$("#customhtmlDisclosure").prepend("<i></i>");})();(function(){$(".ancillary-component").find(".ancillary-item").each(function(e){var f=$(this).attr("id");
