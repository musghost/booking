...
b();$("#title-search").append($("#modify-search-trigger"));}(function(){$(".ancillary-content").each(function(){if($(this).find(".ancillary-info").length<1){var f="ancillary-content-",g=$(this).find(".ancillary-name").first(),h=$("<p class='ancillary-info'>");
if($(this).hasClass(f+"AMC")){h.html("");}else{if($(this).hasClass(f+"AMK")){h.html("Get exclusive experiences and services in your destination, just with a call.");
}else{if($(this).hasClass(f+"DPM")){h.html("");}else{if($(this).hasClass(f+"CO2")){h.html("");}else{if($(this).hasClass(f+"BG_PCS")){h.html("Save time and money. Buy now at a preferential price.");
}}}}}g.after(h);}});$(".ancillary-price dd").each(function(){var f=$(this).find(".prices-all").first();f.next().html("");f.children().first().contents().filter(function(){return this.nodeType===3;
}).remove();});})();c=setTimeout(function(){a();},300);};var c=setTimeout(function(){a();},300);};