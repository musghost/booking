...
}else{$(this).prepend("<span class='step'>B</span>");}});$(".flight-info h2").eq(0).html("Departure Flight Options");$(".flight-info h2").eq(1).html("Return Flight Options ");
...
}}},i=[],h=function(n){var m=$("<div class='flight-info-sidebar'>"),k='<div class="head"><h2><i></i>Flight Information</h2></div>',j=$(".component-section.cart-pax").find("span"),l="",o=$("<h3 class='passenger'>");
...
p.append(x);p.append(y);});if(s===0||s<0){t.html("Nonstop");}else{if(s==1){t.html("1 layover");}else{t.html(s+" layovers");}}p.append(t);m.append(p);});
...
l.append(j);return l;};$(".cart-departing, .cart-arriving").each(function(){var k=[],n=$(this).find("li.city"),o=$(this).find("dd"),j=0,m=$(this).find("h4").first().html(),l=n.length-2;
n.each(function(q){var u=$(this).clone(),t=n.eq(q-1).clone();u.children().remove();t.children().remove();u=$.trim(u.html()).substr(1,3);t=$.trim(t.html()).substr(1,3);
if(q>0){var s=f(o.eq(j).html().split(" ")[2]),r=o.eq(j++).html().substr(0,7),p=o.eq(j++).html().substr(0,7);destination={from:t,to:u,date:s.day+", "+s.month,hourDep:r,hourArr:p};
k.push(destination);}});departing={stopover:l,destinations:k,actionTo:m};i.push(departing);});h(i);$("#modify-search-trigger").addClass("modify");};var a=function(){var f=$("#booking-process li").last();
...
if(n===0||n<0){l.html("Nonstop");}else{if(n===1){l.html(n+" layover");}else{l.html(n+"layovers");}}if($(this).attr("id")==="flight-advisory-both-row"){$(this).remove();
...
});if(p===0){var g=$(this).find("th");g.eq(1).hide();g.eq(2).hide();g.eq(3).hide();g.eq(4).hide();g.eq(0).width("190px");}else{var i=$(this).find("td");
i.eq(4).hide();i.eq(1).hide();i.eq(2).hide();i.eq(3).hide();var o=i.eq(2).find("span").html(),m=i.eq(3).find("span").html(),h=$.trim(i.eq(4).html()),n=parseInt(h);
...
if(n===0||n<0){k.html("Nonstop");}else{if(n===1){k.html(n+" layover");}else{k.html(n+"layovers");}}if($(this).attr("id")==="flight-advisory-outbound-row"){$(this).remove();
return;}i.eq(0).prepend(l,k);var j=$(this).find("a").html();$(this).find("a").addClass("info-plus").html("<i></i><span>Additional information ("+j+")</span>");
...
if(n===0||n<0){k.html("Nonstop");}else{if(n===1){k.html(n+" layover");}else{k.html(n+"layovers");}}if($(this).attr("id")==="flight-advisory-inbound-row"){$(this).remove();
return;}i.eq(0).prepend(l,k);var j=$(this).find("a").html();$(this).find("a").addClass("info-plus").html("<i></i><span>Additional information ("+j+")</span>");
}});$("#cart-component .component-top").append($("#fare-rules-anchor"),$("#modify-search-trigger"));$(".flight-info-sidebar").remove();b();$(".flight-info-sidebar").first().append($("#fare-rules-anchor"));
...
i.eq(4).hide();i.eq(1).hide();i.eq(2).hide();i.eq(3).hide();var o=i.eq(2).find("span").html(),m=i.eq(3).find("span").html(),h=$.trim(i.eq(4).html()),n=parseInt(h);
var l=$("<ul class='airp'>"),k=$('<h3 class="scale">');l.html("<li><span>"+c[0]["nodea"]+"</span><span>"+o+'</span></li><li class="middle"><i></i></li><li><span>'+c[0]["nodeb"]+"</span><span>"+m+"</span></li>");
if(n===0||n<0){k.html("Nonstop");}else{if(n===1){k.html(n+" layover");}else{k.html(n+"layovers");}}if($(this).attr("id")==="flight-advisory-both-row"){$(this).remove();
return;}i.eq(0).prepend(l,k);var j=$(this).find("a").html();$(this).find("a").addClass("info-plus").html("<i></i><span>Additional information ("+j+")</span>");
