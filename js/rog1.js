...
function iChooChooChooseYou(selectID, item, citySelect, citySelected, cityInput){
	...
	if(selectID == 1){
		...
		if(tabNum == 2)
			...
			depRegions.sort(sort_by('country', false));
	}
	if(selectID == 2){
		cityKey2 = item.key;
		if(tabNum == 1)
...
function loadModalCities(airportsArray, region, selectID, citySelect, citySelected){
	...
	if(selectID == 2){
		...
		airportsTemp.sort(sort_by('label', false));
		airportsTemp.sort(function(a,b){
...
$('.cityModal').click(function(){
               ...
               if(selectID == 1){
                       ...
                       depRegions.sort(sort_by('label', false));
                       /*depRegions.sort(function(a,b){
                       	return a["regionTitle"] > b["regionTitle"];
                       });*/
                       //depRegions.sort();
...
citySelect2.click(function(){
	...
	if(cityKey1!=''){
		...
		alert('Tienes que seleccionar primero la ciudad de salida.');
	}
});
